...
│
├── json/merge/
│   ├── file_analysis_backend_spec.rb    # Runs shared examples on all backends
│   ├── smart_merger_backend_spec.rb     # Runs shared examples on all backends
...
The Java backend may have some limitations in:
- Key/value node extraction from pair nodes
...
bundle exec rspec spec/json/merge/*_backend_spec.rb

# Run :auto backend tests (whatever is available)
bundle exec rspec --tag json_grammar spec/json/merge/*_backend_spec.rb
```
...
For classes that don't depend on tree-sitter backends (like ConflictResolver, MergeResult, etc.), regular unit test files in `spec/json/merge/` are fine. They don't need shared examples or backend contexts.
