...
RSpec.describe Json::Merge::Emitter do
  ...
  describe "#add_comma_if_needed edge cases" do
    it "does not add comma to empty line" do
      emitter = described_class.new
      emitter.emit_blank_line
      # Force needs_comma to be true
      emitter.emit_pair("key", '"value"')
      emitter.emit_blank_line
      # The blank line should stay blank
      expect(emitter.lines).to include("")
    end

    it "does not duplicate comma on already comma-terminated line" do
      emitter = described_class.new
      emitter.emit_object_start
      emitter.emit_pair("first", '"value"')
      # Manually add comma (simulate edge case)
      emitter.lines[-1] = emitter.lines[-1] + ","
      emitter.emit_pair("second", '"value"')
      # Should not have double comma
      expect(emitter.lines.any? { |l| l.include?(",,") }).to be false
    end

    it "does not add comma after opening bracket" do
      emitter = described_class.new
      emitter.emit_array_start
      # First element should not need comma
    ...
    it "does not add comma when lines array is empty" do
      ...
      emitter.emit_object_start
      emitter.emit_pair("key", '"value"')
      # Clear lines to simulate edge case
      emitter.lines.clear
      # Now try to emit something - should not crash
      emitter.emit_pair("another", '"value"')
      expect(emitter.lines).not_to be_empty
    end

    it "does not add comma after opening brace" do
      emitter = described_class.new
      emitter.emit_object_start
      # The opening brace line should not get a comma
      expect(emitter.lines.last.rstrip).to end_with("{")
      emitter.emit_pair("key", '"value"')
      # Now opening brace should still not have comma after it
      expect(emitter.lines.first.rstrip).to end_with("{")
    end

    it "handles line that ends with opening bracket" do
      emitter = described_class.new
      emitter.emit_array_start
      expect(emitter.lines.last.rstrip).to end_with("[")
      # Emitting element should not add comma before first element
      emitter.emit_array_element('"first"')
      # First line should still end with [
      expect(emitter.lines.first.rstrip).to end_with("[")
    end
  end
end
